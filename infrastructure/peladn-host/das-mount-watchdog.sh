...
# down karakeep wedges (HTTP 502) and the n8n-driven PVC backups can't write.
...
if [ "$host_ok" = "yes" ] && ! systemctl is-active --quiet nfs-server; then
  ...
  if systemctl start nfs-server && exportfs -ra; then
    notify "NFS auto-recover on Peladn" "nfs-server was down after a DAS flap; restarted it and re-exported. karakeep + the k8s PVC backups depend on it." 6
  else
