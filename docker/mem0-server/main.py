...
REST wrapper around mem0ai. Endpoints are stable across the 0.1.x -> 2.x upgrade
so existing clients (CLAUDE.md curl, the ~/.claude/mem0-mcp.py bridge, Hermes)
work unchanged. mem0ai 2.x specifics handled here:
  - Memory.search(): user query -> `top_k` + `filters={}` (was `limit` + user_id kwargs)
